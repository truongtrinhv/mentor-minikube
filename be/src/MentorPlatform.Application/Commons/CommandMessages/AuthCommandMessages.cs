namespace MentorPlatform.Application.Commons.CommandMessages;

public static class AuthCommandMessages
{
    public const string EditingUserProfileSuccessfully = "Editing profile successfully";    
    public const string LogoutSuccessfully = "Logout successfully";
    public const string RegisterSuccessfully = "Registered account successfully.";
    public const string VerifyEmailSuccessfully = "Verify email successfully";
    public const string VerifyForgotPasswordCodeSuccessfully = "Verify forgot password code successfully";
    public const string ResetPasswordSuccessfully = "Reset password successfully";
    public const string ChangePasswordSuccessfully = "Change password successfully";
    public const string ResendVerifyEmailSuccessfully = "Resend verify email successfully";
}
